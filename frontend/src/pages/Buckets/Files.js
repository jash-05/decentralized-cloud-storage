...
const Files = () => {
		...
		if (storageBackend === "web3") {
			...
			try {
				const res = await axios.delete(
					"http://localhost:8080/storj/file/deleteFile",
					{ params: { fileId: fileId, bucketId: bucketId } }
				);
	...
	const getFiles = async () => {
		...
		const res = await makeAxiosRequest(
			HTTP_METHODS.GET,
			bucketData.StorageBackend === "web3"
				? BACKEND_NAMES.WEB3
				: BACKEND_NAMES.STORJ,
			ROUTE_GROUPS.BUCKET,
			ROUTE_PATHS.GET_FILES,
			null,
			{ bucketId: bucketData?.ID }
		);
		console.log(res);
	...
	const filterData = (query) => {
		...
		const updateData = data.filter((file) =>
		...
		<div className="files-wrapper">
			...
			<div className="buckets-list-wrapper">
				<BasicTable
					...
					storageBackend={bucketData?.StorageBackend}
				/>
			</div>
			<Backdrop
				sx={{
