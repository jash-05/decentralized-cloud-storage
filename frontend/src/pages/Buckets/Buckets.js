...
const Buckets = () => {
	...
	const handleCreateNewBucket = async (event) => {
		...
		if (network === "web3") {
			...
			res = await makeAxiosRequest(
				HTTP_METHODS.POST,
				BACKEND_NAMES.WEB3,
				ROUTE_GROUPS.BUCKET,
				ROUTE_PATHS.CREATE_BUCKET,
				payload,
				null
			);
		} else {
	...
	const [filteredData, setFilteredData] = useState(bucketsData);
	...
	const filterData = (query) => {
		...
		const updateData = bucketsData.filter((bucket) =>
			bucket.BucketName.toLowerCase().includes(query.toLowerCase())
		);
	...
	return (
		<div className="buckets-wrapper">
			<BasicModal
				...
				handleNameChange={handleBucketNameChange}
				handleCreateNewBucket={handleCreateNewBucket}
			...
			<div className="buckets-header">
				<h1>Buckets</h1>
				<Button
					icon={<AddCircleOutlineOutlinedIcon sx={CustomIconStyle} />}
					type="Button"
					text="Create Bucket"
					style={{
						minWidth: "200px",
			...
			<div className="bucket-search-wrapper">
				<InputField
					...
					type="text"
					handleNameChange={(e) => filterData(e.target.value)}
				/>
			</div>
			<br />
			<div className="buckets-list-wrapper">
				<BasicTable
					...
					handleEmptyBucket={handleEmptyBucket}
					options={options}
				/>
			</div>
		</div>
